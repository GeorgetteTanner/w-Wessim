...
### This program is a modified version of Wessim (originally developed from GemSim). Modified lines are marked with #GT
...
def main(argv):
    ...
    for seq	in matchdic: #GT
		if type(matchdic[seq][0])==int: #GT
			...
			n=len(matchdic[seq]) #GT
  		matchweights[seq]=n #GT
	sequences=[k for k in matchweights for x in range(matchweights[k])] #GT
	...
	#choose read length
	if readlength=='d':
		#rdlog.info('Using empirical read length distribution') #GT
		print('Using empirical read length distribution') #GT
		lgth=[]
		keys=rdLenD.keys()
		keys.sort()
		for k in keys:
	...
	gcVector = getFragmentUniform(fref, matchkeys, matchdic, isize, 1000, bind,sequences) #GT
#	print gcVector
#	u1, u2, newSD, m1, m2 = generateMatrices(isd, isize, gcVector)
	...
	seqgenome = "g1"
	while i < readend+1:
		 #key = pickonekey(matchkeys) #GT
		key = pickonekey(sequences) #GT
...
#def pickonekey(matchkeys): #GT
	#r = int(random.uniform(0, len(matchkeys)-1)) #GT
	#key = matchkeys[r] #GT
	#return key #GT

def pickonekey(sequences): #GT
	key = random.choice(sequences)  #GT
	return key #GT
...
def getFragmentUniform(fref, matchkeys, matchdic, mu, total, bind,sequences):########################################################################################################
	result = []
	ins = mu
	i = 0
	seq = ""
