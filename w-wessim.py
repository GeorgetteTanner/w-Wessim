...
def main(argv):
	t0 = time()
	arguline = " ".join(argv)
	parser = argparse.ArgumentParser(description='w-Wessim: Weighted Whole Exome Sequencing SIMulator - modified from Wessim (Probe-based version)', prog='w-Wessim', formatter_class=argparse.RawTextHelpFormatter)
	...
	for t in range(0, threadnumber):
		...
		command = "python __sub_w-wessim.py " + arguline + " -1 " + str(readstart) + " -2 " + str(readend) + " -i " + str(t+1)
